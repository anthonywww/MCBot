...
public class NBTTagCompound extends NBTBase {
	...
	void write(DataOutput par1DataOutput) throws IOException {
		...
		for (Iterator<NBTBase> iterator = tagMap.values().iterator(); iterator.hasNext(); NBTBase.writeNamedTag(nbtbase,
				par1DataOutput)) {
	...
	public NBTBase copy() {
		...
		for (Iterator<String> iterator = tagMap.keySet().iterator(); iterator.hasNext(); nbttagcompound.setTag(s,
				tagMap.get(s).copy())) {
			s = iterator.next();
		}
